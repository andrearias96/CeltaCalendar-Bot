...
def normalize_text(text):
    ...
    text = re.sub('<[^<]+?>', '', text)
    # Normalización agresiva de espacios para comparación
...
def force_kill_chrome():
    ...
    try:
        if os.name == 'posix':
            ...
            # ESPERA DE SEGURIDAD PARA LIBERAR PUERTOS
            time.sleep(2) 
    except Exception:
        pass
...
def scrape_besoccer_info(driver, match_link):
    ...
    stadium = None
    try:
        # TIMEOUT CORTO PARA EVITAR ZOMBIES: Solo 10s para cargar estadio
...
def run_sync():
    ...
    try:
        ...
        for i, match in enumerate(matches):
            ...
            if tv_info_short and not is_finished: 
